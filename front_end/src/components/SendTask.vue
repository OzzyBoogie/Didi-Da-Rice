<template>
    ...
    <ConfirmDialog v-model="showDialog" :form="form" @emitOrder="emitOrder"
                   @cancelDialog="cancelDialog"></ConfirmDialog>
</template>
...
function emitOrder() {
    ...
    data.append('area', ruleForm.area)
    data.append('from', ruleForm.fromAddress)
    data.append('to', ruleForm.toAddress)
    data.append('delivery', ruleForm.delivery ? '1' : '0')  // 已送达时值为'1'
    data.append('date', ruleForm.date1)
    data.append('time', ruleForm.date2)
    data.append('desc', ruleForm.desc)
    axios.post(url + '/emit_order', data).then(function (response) {
        if (response.data['code'] == 200) {
            showMessage('下单成功！', 'success')
            showDialog.value = false
        } else {
            showMessage("下单失败！", "error")
        }
    })
}
</script>
